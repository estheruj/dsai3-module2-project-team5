SELECT 
  ...
  CAST(FORMAT_DATE('%Y%m%d',O.order_purchase_timestamp) AS INT64) AS purchase_date_key,
  CAST(FORMAT_DATE('%Y%m%d',O.order_delivered_customer_date) AS INT64) AS delivered_date_key,
